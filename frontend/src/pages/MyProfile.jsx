...
const MyProfile = () => {
  ...
  const updateUserProfileData = async () => {
    try {
      ...
      // Ensure backendUrl is a valid string
      console.log("Backend URL:", backendUrl); // Log the URL to check if it's correct
        ...
        isEdit ?
          <label htmlFor="image">
            <div className='inline-block relative cursor-pointer'>
              ...
              <img className='w-10 absolute bottom-12 right-12 rounded opacity-75' src={image ? '' : assets.upload_icon} alt="" />
            </div>
  ...
  {
    isEdit ?
      <input className='bg-gray-100 max-w-52' type="email" value={userData.email} onChange={e => setUserData(prev => ({ ...prev, email: e.target.value }))} /> :
